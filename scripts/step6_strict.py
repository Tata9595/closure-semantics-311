"""
步骤 6：严格口径相关性的稳健性检验（论文 §6.4 核心）

背景：
  step5 发现八个设定中唯有「类别边界 严格口径」给出显著相关（ρ=0.340，n=76），
  其余七个均接近零。该结果若只在 Δ=30 + 泊松零模型这一个组合下成立，
  则不足以支撑结论；必须在多个设定下交叉验证。

本脚本在 5×2×2 = 20 个设定下重算严格口径与宽口径的 ρ：
    观察窗   Δ = 7 / 14 / 30 / 60 / 90 天
    零模型   泊松 / 经验时移
    NR 口径  宽（B–G）/ 严格（B,C,E,F）

并额外检验：
  - 逐类别相关性：单独用 B、C、D、E、F、G 各自的占比与超额复发求 ρ
    → 定位究竟是哪些类别携带信号、哪些在稀释信号
  - 散点图数据：严格口径下的类型级散点，供论文图 3 使用

前置：step5 已运行（库中需有 rec_w7…rec_w90、rec_s91…rec_s365、labeled 表）

用法：
    python step6_strict.py
"""
...
def main():
    ...
    print("  这张表为「为何严格口径更强」提供直接证据，是论文 §6.4 的关键论据。")
