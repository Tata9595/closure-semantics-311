"""
步骤 4：结案语义分类 + 两路径交叉验证（论文 §4.1、§6.1、§6.4）

分两步用：

  ① python step4_classify.py --extract
     统计结案说明模板的频次，生成待标注表 out/I1_templates_to_label.csv
     表中已按关键词给出 suggested_code，你只需核对并填 code 列

  ② python step4_classify.py --apply
     读回填好的表，计算：
       - 非解决型结案占比（总体 / 按部门 / 按类型）
       - 与超额复发的交叉验证（Spearman 相关 + 散点图数据）

前置：step2 已建库、step3 已跑过（需要库里的 excess 表）

类别代码：
  A 实质处置      已修复 / 开传票 / 认定违规成立
  B 到场未发现    未观察到证据 / 未发现该状况 / 不构成违规
  C 责任人已离开  到场时责任人已离开
  D 无需行动      认定无需介入 / 无需进一步处理
  E 无法进入      未能进入住户或场所 / 检查未完成
  F 重复工单      判定为已有工单的重复件
  G 转出/告知     转他部门 / 仅提供信息
  X 无法判定      需读全文或语义不清（计入未归类）
"""
